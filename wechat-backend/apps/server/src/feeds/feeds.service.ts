...
import { buildWechatArticleUrl } from '@server/utils/wechat';
...
export class FeedsService {
  ...
  async renderFeed({
    ...
    const feedUpdatedAt =
      articles[0]?.publishTime || feedInfo.syncTime || feedInfo.updateTime;
    ...
    const feed = new Feed({
      ...
      copyright: '',
      updated: new Date(feedUpdatedAt * 1e3),
      generator: 'WeWe-RSS',
    ...
    const mapper = async (item) => {
      ...
      const link = buildWechatArticleUrl(id);
  ...
  async updateFeed(id: string) {
    return this.trpcService.refreshMpArticlesAndUpdateFeed(id);
  }
}
