...
const Feeds = () => {
  ...
  const syncRefreshState = async () => {
    await Promise.all([
      refetchFeedList(),
      queryUtils.account.list.invalidate(),
      queryUtils.article.list.invalidate(),
      queryUtils.feed.isRefreshAllMpArticlesRunning.invalidate(),
    ]);
  };
  ...
  const formatPublishTime = (publishTime?: number | null) => {
    if (!publishTime) {
      return 'unknown';
    }

    return dayjs(publishTime * 1e3).format('YYYY-MM-DD HH:mm:ss');
  };

  const showRefreshResult = (result: unknown, mpName?: string) => {
    if (!result) {
    ...
    if (typeof result === 'object' && result !== null && 'results' in result) {
      const aggregateResult = result as unknown as {
        totalInsertedCount: number;
      };

      if (aggregateResult.totalInsertedCount > 0) {
        toast.success('Refresh completed', {
          description: `Inserted ${aggregateResult.totalInsertedCount} new articles.`,
        });
        return;
      }

      toast.info('Checked all feeds. No new articles.');
      return;
    }
    ...
    if (
      ...
      if (singleResult.insertedCount > 0) {
        toast.success('Refresh completed', {
          description: `${mpName || 'This feed'} added ${
            singleResult.insertedCount
          } new articles.`,
        });
        return;
      }

      toast.info('Checked. No new articles.', {
        description: `Current latest article time: ${formatPublishTime(
          singleResult.latestLocalPublishTime ||
            singleResult.latestRemotePublishTime,
        )}`,
      });
      return;
    }
  ...
  const isActive = (key: string) => currentMpId === key;
              ...
              <ListboxSection className="overflow-y-auto h-[calc(100vh-260px)]">
                {feedData.items.map((item) => (
                  <ListboxItem
                    ...
                    key={item.id}
                    startContent={<Avatar src={item.mpCover} />}
                  >
                    {item.mpName}
                  </ListboxItem>
                ))}
                  ...
                  <Link
                    ...
                    onClick={async (ev) => {
                      ...
                      try {
                        ...
                        showRefreshResult(result, currentMpInfo.mpName);
                      } catch (error) {
                        toast.error('Refresh failed', {
                          description: getErrorMessage(error),
                        });
                      } finally {
                        await syncRefreshState();
                      }
                    }}
                  >
                    ...
                    <Tooltip
                      content={
                        inProgressHistoryMp?.id === currentMpInfo.id
                          ...
                          : '历史文章需要分批次拉取，请耐心等待，频繁调用可能会导致一段时间内不可用'
                      }
                      color={
                        inProgressHistoryMp?.id === currentMpInfo.id
                          ? 'primary'
        ...
        <ModalContent>
          {(modalOnClose) => (
            <>
              ...
              <ModalFooter>
                <Button color="danger" variant="flat" onPress={modalOnClose}>
                  取消
                </Button>
                <Button
                  color="primary"
